...
function build_package {
    ...
    rm -rf ${TMP_DIR} ${PYTHON}-${PACKAGE}-${VERSION}.zip
    mkdir ${TMP_DIR}
    cd  ${TMP_DIR}
    ...
    echo ${PIP} install --verbose --use-wheel --target ${TARGET_DIR} "${PACKAGE}==${VERSION}"
    ${PIP} install --use-wheel --target ${TARGET_DIR} "${PACKAGE}==${VERSION}"
    deactivate

    TARGET_DIR=${ENV}/packaged
    #cd ${TARGET_DIR} && tar -zcvf ../../../${PYTHON}-${PACKAGE}-${VERSION}.tar.gz . && cd ../../..
    cd ${TARGET_DIR} && zip -r ../../../${PYTHON}-${PACKAGE}-${VERSION}.zip . && cd ../../..
    rm -r ${TMP_DIR}
